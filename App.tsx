...
// --- INTEGRAÇÃO FIREBASE OFICIAL (BLOQUEIO DE SEGURANÇA) ---
...
const RecuperarID = ({ onBack }: { onBack: () => void }) => {
  ...
  const buscarID = async (e: React.FormEvent) => {
    ...
    } catch (err) { setErro('Erro na conexão com a central.'); }
  };
  return (
    <div className="min-h-screen bg-black text-white flex items-center justify-center p-6 animate-in fade-in duration-500">
      ...
      <div className="max-w-md w-full border border-zinc-800 bg-zinc-950 p-8 rounded-[40px] shadow-2xl">
        ...
        <form onSubmit={buscarID} className="space-y-4 mt-6">
          <input type="email" placeholder="seu@email.com" className="w-full bg-black border border-zinc-800 p-5 rounded-2xl focus:border-[#D4AF37] outline-none transition-all" value={email} onChange={(e) => setEmail(e.target.value)} required />
        ...
        {resultado && <div className="mt-8 p-6 bg-zinc-900 border border-[#D4AF37] rounded-3xl text-center shadow-inner animate-bounce"><p className="text-xs text-zinc-500 uppercase mb-2">Sua Credencial:</p><p className="text-3xl font-mono font-black text-white">{resultado}</p></div>}
  ...
  return (
    <div className="min-h-screen bg-black text-white p-8 animate-in fade-in duration-500">
      ...
      {loading ? <p className="text-center animate-pulse text-[#D4AF37]">Acessando registros...</p> : (
        <div className="max-w-5xl mx-auto border border-zinc-800 rounded-3xl bg-zinc-950 shadow-2xl">
          <table className="w-full text-left"><thead className="bg-zinc-900 text-[10px] uppercase text-zinc-500 tracking-widest"><tr><th className="p-6">ID Agente</th><th className="p-6">E-mail</th><th className="p-6">Status</th></tr></thead>
            <tbody className="text-sm">{agentes.map(a => <tr key={a.id} className="border-b border-zinc-900"><td className="p-6 font-mono text-[#D4AF37] font-bold">{a.id}</td><td className="p-6">{a.email}</td><td className="p-6 text-green-500 font-bold uppercase text-[10px]">Ativo</td></tr>)}</tbody>
          </table>
        </div>
      )}
    </div>
...
const LandingPage = ({ onLogin, onRecuperar }: any) => (
 <div className="w-full bg-[#0a0a0a] flex flex-col items-center selection:bg-[#D4AF37] selection:text-black">
  ...
  <main className="w-full max-w-7xl px-8 flex flex-col items-center text-center mt-12 mb-32 mx-auto italic">
   ...
   <p className="text-gray-400 text-lg md:text-2xl font-medium max-w-5xl mb-20 italic leading-relaxed text-center px-4">Rastreie, analise e modele VSLs, criativos e funis que estão gerando milhões. O fim do "achismo" na sua escala digital.</p>
   <section className="w-full max-w-4xl aspect-video bg-[#121212] rounded-[32px] border border-white/10 shadow-2xl relative flex flex-col items-center justify-center group cursor-pointer transition-all hover:border-[#D4AF37]/30 mb-32"><div className="bg-[#D4AF37] p-6 rounded-full shadow-[0_0_40px_rgba(212,175,55,0.3)] group-hover:scale-110 transition-transform mb-6"><Play size={40} fill="black" className="text-black ml-1" /></div><p className="text-white font-black uppercase text-[10px] md:text-xs tracking-[0.25em]">Descubra como rastreamos ofertas escaladas em tempo real</p></section>
   <div className="grid grid-cols-1 md:grid-cols-2 gap-10 max-w-5xl mx-auto mb-40 text-left">
    <div className="bg-[#121212] border border-white/5 rounded-[40px] p-12 flex flex-col shadow-2xl"><h3 className="text-[#D4AF37] font-black uppercase text-xl mb-4 italic">Plano Mensal</h3><p className="text-5xl font-black text-white mb-10">R$ 197 <span className="text-sm font-normal text-zinc-500">/mês</span></p><button onClick={() => window.open(KIWIFY_MENSAL)} className="w-full py-5 bg-white text-black font-black text-xl rounded-2xl hover:scale-105 transition-all shadow-xl uppercase italic">QUERO ACESSO MENSAL</button></div>
    <div className="bg-white text-black rounded-[40px] p-12 flex flex-col scale-105 border-t-8 border-[#D4AF37] shadow-2xl shadow-[#D4AF37]/20"><h3 className="text-[#D4AF37] font-black uppercase text-xl mb-4 italic">Plano Trimestral</h3><p className="text-5xl font-black mb-10">R$ 497 <span className="text-sm font-normal text-gray-400">/tri</span></p><button onClick={() => window.open(KIWIFY_TRIMESTRAL)} className="w-full py-5 bg-black text-[#D4AF37] font-black text-xl rounded-2xl animate-btn-pulse uppercase italic">ASSINAR TRIMESTRAL</button></div>
   </div>
   <div className="w-full max-w-5xl mx-auto mb-40 border border-[#D4AF37]/30 rounded-[40px] p-16 flex flex-col md:flex-row items-center gap-12 text-left bg-zinc-950 shadow-2xl shadow-[#D4AF37]/10"><div className="w-32 h-32 md:w-44 md:h-44 rounded-full border-4 border-[#D4AF37] flex items-center justify-center text-6xl md:text-8xl font-black text-[#D4AF37]">7</div><div className="space-y-4 flex-1"><h2 className="text-white text-3xl md:text-5xl font-black uppercase italic tracking-tighter">GARANTIA INCONDICIONAL DE <span className="text-[#D4AF37]">7 DIAS</span></h2><p className="text-gray-400 text-lg md:text-xl font-medium italic leading-relaxed">Se em até 7 dias você não sentir que a plataforma é para você, devolvemos 100% do seu investimento. Risco zero para sua operação.</p></div></div>
...
const App = () => {
  ...
  const fetchOffers = async () => {
   try {
    ...
    const parsed: Offer[] = lines.slice(2).map((l, i) => {
     ...
     if (!v[1]) return null;
     return { id: v[0] || String(i), title: v[1], niche: v[2], productType: v[3], description: v[4], coverImage: v[5], vslLinks: (v[8] || '').split(',').map(u => ({ label: 'VSL Principal', url: u.trim() })), transcriptionUrl: v[10], creativeEmbedUrls: (v[11] || '').split(',').filter(Boolean), creativeDownloadUrls: (v[12] || '').split(',').filter(Boolean), facebookUrl: v[13], pageUrl: v[14], status: (v[19] || '').toUpperCase() } as Offer;
 ...
 return (
  <div className="flex min-h-screen bg-black text-white selection:bg-[#D4AF37] selection:text-black">
   <style dangerouslySetInnerHTML={{ __html: STYLES }} />
   <aside className="w-72 bg-[#121212] border-r border-white/5 h-screen sticky top-0 p-10 flex flex-col shadow-2xl">
    <div className="flex items-center space-x-3 mb-16 cursor-pointer" onClick={() => setSelectedOffer(null)}><div className="bg-[#D4AF37] p-2 rounded-xl"><Eye className="text-black" size={24} /></div><span className="text-xl font-black italic uppercase leading-none">007 SWIPER</span></div>
    <nav className="space-y-4 flex-1">
     ...
     <SidebarItem icon={Tag} label="Todas Ofertas" active={currentPage === 'offers'} onClick={() => {setCurrentPage('offers'); setSelectedOffer(null)}} />
    </nav>
    <div className="pt-8 border-t border-white/5">
      <div className="bg-[#1a1a1a] p-4 rounded-xl mb-4 border border-white/5"><p className="text-zinc-600 text-[8px] font-black uppercase mb-1">ID AGENTE</p><p className="text-[#D4AF37] font-mono text-[9px] font-black truncate uppercase">{agentId}</p></div>
   ...
   <main className="flex-1 p-12">
    <header className="flex justify-between items-center mb-16"><h1 className="text-3xl font-black italic uppercase tracking-tighter italic leading-none">Radar de <span className="text-[#D4AF37]">Inteligência</span></h1><div className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-600" size={16}/><input type="text" placeholder="RASTREAR..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="bg-[#121212] border border-white/5 pl-12 pr-4 py-4 rounded-2xl w-80 font-black text-xs italic focus:border-[#D4AF37] outline-none shadow-inner" /></div></header>
    {loading ? <div className="py-40 text-center animate-pulse text-[#D4AF37] font-black uppercase italic">Infiltrando base de dados...</div> : selectedOffer ? (
     <div className="space-y-12 animate-in fade-in duration-500 italic">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
       <div className="lg:w-[65%] space-y-8">
        <div className="aspect-video bg-black rounded-3xl overflow-hidden shadow-2xl border border-white/5"><iframe className="w-full h-full" src={getEmbedUrl(selectedOffer.vslLinks[0]?.url)} frameBorder="0" allowFullScreen></iframe></div>
        <div className="bg-[#121212] p-10 rounded-[40px] border border-white/5 shadow-2xl"><h3 className="text-[#D4AF37] font-black uppercase text-[10px] mb-6 tracking-[0.2em] italic border-l-2 border-[#D4AF37] pl-4">Análise Operacional</h3><p className="text-zinc-400 font-medium italic leading-relaxed text-lg">{selectedOffer.description || "Descrição em processamento..."}</p></div>
       </div>
       <div className="lg:w-[35%] bg-[#121212] p-10 rounded-[40px] border border-white/5 h-fit space-y-10 shadow-2xl">
        <h3 className="text-zinc-700 font-black uppercase text-[10px] tracking-[0.3em] italic border-l-2 border-zinc-800 pl-4">Dossiê Técnico</h3>
        ...
        <a href={selectedOffer.pageUrl} target="_blank" className="block w-full py-5 btn-elite rounded-2xl text-center text-sm font-black italic">VISUALIZAR PÁGINA</a>
        <a href={selectedOffer.facebookUrl} target="_blank" className="block w-full py-5 bg-[#1a1a1a] border border-white/10 rounded-2xl text-center text-xs italic font-black text-zinc-500 hover:text-white transition-all">ADS LIBRARY</a>
       </div>
      </div>
      <div className="space-y-8 pt-10">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {selectedOffer.creativeEmbedUrls.map((url:any, i:any) => (<div key={i} className="bg-[#121212] p-5 rounded-[32px] border border-white/5 space-y-5 shadow-2xl hover:border-[#D4AF37]/30 transition-all"><div className="aspect-video bg-black rounded-2xl overflow-hidden"><iframe className="w-full h-full" src={getEmbedUrl(url)} frameBorder="0" allowFullScreen></iframe></div><a href={selectedOffer.creativeDownloadUrls[i]} target="_blank" className="block w-full py-4 bg-[#1a1a1a] border border-[#D4AF37]/20 text-[#D4AF37] rounded-2xl text-center font-black text-[10px] uppercase hover:bg-[#D4AF37] hover:text-black transition-all shadow-inner">Baixar Criativo VIP</a></div>))}
         </div>
      </div>
     </div>
    ) : (
