    ...
    <div className="min-h-screen bg-black text-white flex items-center justify-center p-6 animate-in fade-in">
      ...
      <div className="max-w-md w-full border border-zinc-800 bg-zinc-950 p-10 rounded-[40px] shadow-2xl">
        ...
        <form onSubmit={buscarID} className="space-y-4">
          <input type="email" placeholder="seu@email.com" className="w-full bg-black border border-zinc-800 p-5 rounded-2xl focus:border-[#D4AF37] outline-none text-white font-bold" value={email} onChange={(e) => setEmail(e.target.value)} required />
          <button className="w-full bg-[#D4AF37] text-black font-bold p-5 rounded-2xl hover:bg-white transition-all uppercase italic">Consultar Base</button>
        </form>
        {resultado && <div className="mt-8 p-6 bg-zinc-900 border border-[#D4AF37] rounded-3xl text-center"><p className="text-xs text-zinc-500 uppercase mb-2">Sua Credencial:</p><p className="text-3xl font-mono font-black text-white selection:bg-[#D4AF37]">{resultado}</p></div>}
      </div>
    </div>
  );
};
    ...
    <div className="min-h-screen bg-black text-white p-10 animate-in fade-in">
      ...
      <div className="max-w-5xl mx-auto overflow-hidden border border-zinc-800 rounded-3xl bg-zinc-950">
        <table className="w-full text-left"><thead className="bg-zinc-900 text-[10px] uppercase text-zinc-500 tracking-widest"><tr><th className="p-6">ID Agente</th><th className="p-6">E-mail</th><th className="p-6">Status</th></tr></thead>
          <tbody className="text-sm italic font-bold">{agentes.map(a => <tr key={a.id} className="border-b border-zinc-900"><td className="p-6 font-mono text-[#D4AF37]">{a.id}</td><td className="p-6">{a.email}</td><td className="p-6 text-green-500 font-black">ATIVO</td></tr>)}</tbody>
        </table>
      </div>
    </div>
  );
  ...
  return (
    <div onClick={onClick} className="bg-[#121212] rounded-2xl overflow-hidden group cursor-pointer border border-white/5 hover:border-[#D4AF37]/50 transition-all duration-500 shadow-xl">
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between border-t border-white/5 pt-4"><div className="flex items-center gap-2">{offer.trafficSource.slice(0, 2).map((source, idx) => <TrafficIcon key={idx} source={source} />)}<span className="text-gray-500 text-[9px] font-bold uppercase tracking-widest">{offer.productType}</span></div></div>
      </div>
    </div>
  );
};
...
const LandingPage = ({ onLogin, isSuccess, agentId, onDismissSuccess, onRecover, onAdmin }: any) => (
 <div className="w-full bg-[#0a0a0a] flex flex-col items-center selection:bg-[#D4AF37] selection:text-black overflow-x-hidden italic font-black">
...
const App: React.FC = () => {
 ...
 const checkLogin = async (id: string, silencioso = false) => {
    ...
    try {
        ...
        if (docSnap.exists() && docSnap.data().ativo === true) {
            ...
            const favs = localStorage.getItem(getFavKey(cleanId)); if (favs) setFavorites(JSON.parse(favs));
        } else { if (!silencioso) alert('ACESSO NEGADO ❌'); }
    } catch (e) { console.error(e); }
 };

 const handleLogin = () => { const id = window.prompt("🕵️‍♂️ ID DO AGENTE:"); if (id) checkLogin(id); };
 ...
 const openOffer = (offer: Offer) => { setSelectedOffer(offer); window.scrollTo({ top: 0, behavior: 'smooth' }); };
 const navigateToPage = (page: string) => { setCurrentPage(page); setSelectedOffer(null); setActiveNicheSelection(null); setActiveLanguageSelection(null); setIsMobileMenuOpen(false); };
 ...
 const renderContent = () => {
   ...
   if (selectedOffer) return (
     <div className="animate-in fade-in duration-500"><div className="flex items-center justify-between mb-10"><button onClick={() => setSelectedOffer(null)} className="flex items-center text-gray-500 hover:text-[#D4AF37] font-black uppercase text-xs tracking-widest transition-all group"><div className="bg-[#1a1a1a] p-2 rounded-lg mr-3 group-hover:bg-[#D4AF37] group-hover:text-black transition-all"><ArrowLeft size={16} /></div> Voltar</button></div><div className="flex flex-col lg:flex-row gap-8 items-stretch italic"><div className="w-full lg:w-[62%] space-y-6"><div className="bg-[#121212] p-4 md:p-6 rounded-[32px] border border-white/5 shadow-2xl h-full flex flex-col"><div className="aspect-video rounded-2xl overflow-hidden bg-black border border-white/5 relative z-10 flex-1 shadow-2xl"><VideoPlayer url={selectedOffer.vslLinks[0]?.url} title="VSL Player" /></div><div className="mt-6 p-8 bg-[#1a1a1a] rounded-[32px] border border-white/5"><h3 className="text-[#D4AF37] font-black text-xs italic mb-4 border-l-2 border-[#D4AF37] pl-4 uppercase">Dossiê Técnico</h3><p className="text-zinc-400 font-medium leading-relaxed text-lg whitespace-pre-line">{selectedOffer.description || "Descrição técnica em processamento..."}</p></div></div></div><div className="w-full lg:w-[38%]"><div className="bg-[#121212] p-8 md:p-10 rounded-[40px] border border-white/5 shadow-2xl h-full flex flex-col space-y-8"><h3 className="text-[#D4AF37] font-black uppercase text-xs tracking-widest italic border-l-2 border-[#D4AF37] pl-4">Dossiê da Operação</h3><div className="space-y-6 flex-1">{[{ icon: Tag, label: 'NICHO', value: selectedOffer.niche }, { icon: Lock, label: 'ESTRUTURA', value: selectedOffer.productType }, { icon: Globe, label: 'IDIOMA', value: selectedOffer.language }, { icon: Target, label: 'FONTE', value: selectedOffer.trafficSource[0] }].map((item, idx) => (<div key={idx} className="border-b border-white/5 pb-4"><div className="flex items-center gap-2 text-zinc-600 text-[10px] font-black uppercase mb-1"><item.icon size={14}/> {item.label}</div><p className="text-white font-black uppercase text-base">{item.value}</p></div>))}</div><a href={selectedOffer.pageUrl} target="_blank" className="block w-full py-5 btn-elite rounded-2xl text-center text-sm italic font-black shadow-xl uppercase animate-btn-pulse">Visualizar Página</a><a href={selectedOffer.facebookUrl} target="_blank" className="block w-full py-5 bg-[#1a1a1a] border border-white/10 rounded-2xl text-center text-xs font-black text-zinc-500 hover:text-white transition-all uppercase italic">Ads Library</a></div></div></div></div>
   );
   const filtered = applyEliteFilters(offers);
   switch (currentPage) {
     case 'home': return (<div className="animate-in fade-in duration-700 space-y-16"><div><h2 className="text-2xl font-black text-white uppercase italic mb-8 flex items-center gap-4"><Zap className="text-[#D4AF37]" fill="currentColor" /> OPERAÇÕES EM ESCALA</h2><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">{offers.filter(o => o.trend.trim().toLowerCase() === 'escalando').slice(0, 4).map((o) => <OfferCard key={o.id} offer={o} isFavorite={favorites.includes(o.id)} onToggleFavorite={(e:any) => toggleFavorite(o.id, e)} onClick={() => openOffer(o)} />)}</div></div></div>);
     case 'offers': return (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 animate-in fade-in duration-700">{filtered.map((o) => <OfferCard key={o.id} offer={o} isFavorite={favorites.includes(o.id)} onToggleFavorite={(e:any) => toggleFavorite(o.id, e)} onClick={() => openOffer(o)} />)}</div>);
     case 'favorites': return (<div className="animate-in fade-in duration-700"><h2 className="text-2xl font-black text-white uppercase italic mb-8 flex items-center gap-4"><Star className="text-[#D4AF37]" fill="currentColor" /> SEUS FAVORITOS</h2><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">{offers.filter(o => favorites.includes(o.id)).map((o) => <OfferCard key={o.id} offer={o} isFavorite={true} onToggleFavorite={(e:any) => toggleFavorite(o.id, e)} onClick={() => openOffer(o)} />)}</div></div>);
     default: return null;
   }
 };

 const SidebarContent = () => (
   <div className="p-8 md:p-10 h-full flex flex-col"><div className="flex items-center space-x-3 mb-12 px-2"><div className="bg-[#D4AF37] p-2 rounded-xl shadow-xl shadow-[#D4AF37]/10"><Eye className="text-black" size={24} /></div><span className="text-2xl font-black tracking-tighter text-white uppercase italic leading-none">007 SWIPER</span></div>
     ...
     <div className="mt-8 space-y-3"><SidebarItem icon={LogOut} label="Sair" active={false} onClick={() => {setIsLoggedIn(false); localStorage.removeItem('agente_token');}} variant="danger" /></div>
   </div>
 );
 ...
 return (
   <div className="flex min-h-screen bg-[#0a0a0a] text-white selection:bg-[#D4AF37] selection:text-black">
     ...
     {isLoggedIn ? (
       <>
         ...
         <main className="flex-1 lg:ml-72 relative w-full">
           <header className="h-auto py-6 md:py-8 flex flex-col px-4 md:px-10 bg-[#0a0a0a]/80 backdrop-blur-xl sticky top-0 z-[80] border-b border-white/5 gap-4">
             <div className="flex items-center justify-between gap-4"><div className="flex-1"></div><div className="flex items-center gap-3 bg-[#121212] p-1.5 pr-4 md:pr-6 rounded-[16px] md:rounded-[24px] border border-white/5 shadow-2xl ml-2 md:ml-6 shrink-0"><div className="w-8 h-8 md:w-10 md:h-10 bg-[#D4AF37] rounded-lg md:rounded-xl flex items-center justify-center font-black text-black text-sm md:text-lg shadow-lg">007</div><div className="hidden sm:block"><p className="font-black text-[10px] uppercase tracking-tighter text-white leading-none">Agente Ativo</p></div></div></div>
             {showFilters && (
               <div className="grid grid-cols-1 md:grid-cols-5 gap-4 animate-in fade-in slide-in-from-top-2 duration-500 pb-2">
                 <div className="flex flex-col gap-1.5 w-full">
                    ...
                    <div className="relative w-full">
                        <input type="text" placeholder="Pesquisar..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-[#121212] border border-white/10 rounded-xl pl-4 pr-10 py-2 text-[10px] md:text-[11px] font-black uppercase text-white outline-none hover:border-[#D4AF37] transition-all h-[38px] placeholder:text-zinc-700 focus:border-[#D4AF37]" />
       ...
       <LandingPage onLogin={handleLogin} onRecover={() => setShowRecuperar(true)} onAdmin={() => setShowAdmin(true)} isSuccess={isSuccess} agentId={newlyGeneratedId} onDismissSuccess={() => setIsSuccess(false)} />
     )}
   </div>
 );
};
