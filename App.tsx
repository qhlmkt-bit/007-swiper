...
const VideoPlayer: React.FC<{ url: string; title?: string; type?: 'vsl' | 'creative' }> = ({ url, title, type = 'vsl' }) => { 
  ...
  let content;
  // Detecta qualquer link do Drive (thumbnail, /d/ID, export=download) ou extensão de imagem
  const isDriveImage = trimmed.includes('drive.google.com/thumbnail') 
    || (trimmed.includes('drive.google.com') && trimmed.includes('export=download'))
    || (trimmed.includes('drive.google.com') && trimmed.includes('/d/'));
  if (trimmed.match(/\.(jpeg|jpg|gif|png|webp)(\?.*)?$/i) || isDriveImage) {
    const imgSrc = getDriveDirectLink(trimmed); // converte qualquer formato Drive para thumbnail seguro
    content = <img src={imgSrc} alt={title || 'Media'} className="w-full h-full object-contain bg-black" loading="lazy" decoding="async" />;
...
const App: React.FC = () => {
 ...
 const openOffer = (offer: Offer) => {
  ...
  setActiveVslIndex(0); // ← sempre começa no VSL 1
