...
const VideoPlayer: React.FC<{ url: string; title?: string; type?: 'vsl' | 'creative' }> = ({ url, title, type = 'vsl' }) => { 
  ...
  let content;
  // TRAVA DE SEGURANÇA: Se por acaso cair um link de imagem aqui, ele renderiza como imagem e não quebra a tela
  ...
  return <div className="w-full h-full flex items-center justify-center bg-black rounded-2xl overflow-hidden shadow-2xl relative">{content}</div>;
};
     ...
     <div className="animate-in fade-in duration-700 space-y-16 md:space-y-20">
       <div>
         ...
         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
           ...
           {filtered.filter(o => o.trend.trim().toLowerCase() === 'escalando').slice(0, 4).map((o) => <OfferCard key={o.id} offer={o} isFavorite={favorites.includes(o.id)} onToggleFavorite={(e:any) => toggleFavorite(o.id, e)} onClick={() => openOffer(o)} />)}
         </div>
